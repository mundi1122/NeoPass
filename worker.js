...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    // Get selected text - try selectionText first, then grab from page (for terminals/iframes)
    let selectedText = info.selectionText;
    if (!selectedText && (info.menuItemId === 'search' || info.menuItemId === 'solveMCQ' || info.menuItemId === 'nptel')) {
        try {
            const results = await chrome.scripting.executeScript({
                target: { tabId: tab.id, allFrames: true },
                func: () => {
                    // Try native selection first
                    let text = window.getSelection().toString().trim();
                    if (text) return text;
                    // Try xterm.js terminal selection
                    const terminalEl = document.querySelector('.xterm-helper-textarea');
                    if (terminalEl) {
                        const term = document.querySelector('.xterm');
                        if (term && term._core) {
                            text = term._core.buffer.active.translateBufferLineToString(0);
                        }
                    }
            ...
            for (const result of results) {
                if (result.result) { selectedText = result.result; break; }
            }
        } catch (e) {
            console.log('Failed to get selection from frames:', e);
        }
    }

    if (info.menuItemId === 'search' && selectedText) {
        // Show spinner toast while processing
        showSpinnerToast(tab.id, 'Analyzing question...');

        queryRequest(selectedText).then(response => {
    ...
    } else if (info.menuItemId === 'search' && !selectedText) {
        showToast(tab.id, 'No text selected. Select text first, then right-click.', true);
    }

    if (info.menuItemId === 'solveMCQ' && selectedText) {
        // Show spinner toast while processing
        showSpinnerToast(tab.id, 'Analyzing MCQ question...');

        queryRequest(selectedText, true).then(response => {
    ...
    } else if (info.menuItemId === 'solveMCQ' && !selectedText) {
        showToast(tab.id, 'No text selected. Select text first, then right-click.', true);
    }
...
async function getSelectedText() {
    // 1. Try browser's native selection (works for normal text, textareas, inputs)
    const selectedText = window.getSelection().toString().trim();
    if (selectedText) return selectedText;

    // 2. Try getting selection from Monaco/ACE editors via MAIN world hooks
    try {
        const editorText = await new Promise((resolve) => {
            let done = false;
            const handler = (event) => {
                if (done || event.source !== window || !event.data) return;
                if (event.data.type === 'NEOPASS_EDITOR_SELECTION_RESULT') {
            ...
            // Ask both hooks — only the active one will return text
            window.postMessage({ type: 'NEOPASS_MONACO_GET_SELECTION' }, '*');
            window.postMessage({ type: 'NEOPASS_ACE_GET_SELECTION' }, '*');
            // Timeout after 300ms
            setTimeout(() => {
                if (!done) {
                    done = true;
        ...
        switch (aiProvider) {
            ...
            case 'custom':
                ...
                headers = {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${apiKey}`,
                    // Required by OpenRouter and other aggregators
                    'HTTP-Referer': 'https://freeneopass.vercel.app',
                    'X-Title': 'NeoPass'
                };
                requestBody = {
