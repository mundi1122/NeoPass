...
(function () {
    ...
    document.addEventListener('copy', function (event) {
        ...
        // Allow copy events from Monaco or ACE code editors
        ...
        if (event.target && (
            event.target.closest && (
                ...
                event.target.closest('.ace_editor')
            )
        )) {
            return;
        }
    ...
    document.addEventListener('keydown', async function (event) {
        if ((event.ctrlKey || event.metaKey) && !event.altKey && !event.shiftKey && event.key === 'c') {
            ...
            // Let Monaco / ACE editors handle Ctrl+C natively
            const active = document.activeElement;
            if (active && (
                active.closest('.monaco-editor') ||
                active.closest('.ace_editor') ||
                ...
                return; // Don't intercept — editor handles it
            }
